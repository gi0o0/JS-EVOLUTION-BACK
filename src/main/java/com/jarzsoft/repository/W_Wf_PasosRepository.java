...
public interface W_Wf_PasosRepository extends JpaRepository<W_Wf_Pasos, W_Wf_PasosPK> {
	...
	@Query(value = "select step.* from w_wf_mov mov inner join w_wf_pasos step on step.id_paso =mov.id_paso and step.id_wf = mov.id_wf where numero_radicacion =:numRad order by id_paso ", nativeQuery = true)
