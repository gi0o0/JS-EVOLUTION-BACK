...
public interface SolCreditoRepository extends JpaRepository<SolCredito, Integer> {
	...
	@Query(value = "select DISTINCT s.numero_radicacion,s.estado from w_wf_mov w, SOL_CREDITO s where w.id_wf = 4 and w.numero_radicacion = s.numero_radicacion and (w.usu_comercial = :user or "
			+ "w.usu_movimiento = :user or w.usuario1 = :user or w.usuario2 = :user or w.usuario3 = :user or w.usuario4 = :user) and (w.id_paso <>'8' and  s.estado not in ('P', 'A')    ) ", nativeQuery = true)
