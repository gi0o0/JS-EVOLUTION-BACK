...
public interface W_Wf_Pas_AutRepository extends JpaRepository<W_Wf_Pas_Aut, W_Wf_Pas_AutPK> {
	...
	@Query(value = "SELECT CASE WHEN p.usuario IS NULL THEN 'false' ELSE 'true' END from W_Bas_Usuario u Left join w_wf_pas_aut p On u.Usuario =p.usuario WHERE u.Usuario=:user AND p.id_paso =:id_paso ANd p.id_wf =:id_wf", nativeQuery = true)
	Boolean findByUserByStep(@Param("id_wf") String id_wf, @Param("id_paso") String id_paso,
